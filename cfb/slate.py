...
HOURS_HIGHLIGHTED    = 3   # how many of those hours get the shaded highlight
                           # Separate from the window on purpose. The window
                           # is how much forecast we SHOW; the highlight is
                           # which hours are actually the game. CFB used
                           # HOURS_GAME_WINDOW for both, so it shaded 4 hours
                           # instead of 3. NFL already split these (see
                           # nfl/slate.py); MLB uses HOURS_GAME = 3.
